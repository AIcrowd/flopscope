...
REGISTRY: dict[str, dict] = {
    ...
    "gradient": {
        ...
        "notes": (
            ...
            "FLOPs each, so the cost is independent of axis length. A single-axis "
            "gradient is exactly 1/ndim of the all-axes gradient and the per-axis "
            "costs sum back to it. A 1-D coordinate-array vararg adds a surcharge on "
            "its own axis: 3*(L-1) when its diffs are bit-exactly uniform, otherwise "
            "3*S*(L-2)//L + 10*(L-2) + 3*(L-1) + 4*S//L with L = f.shape[axis]."
        ),
    },
    "ediff1d": {
        "category": "counted_custom",
