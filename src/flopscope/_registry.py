...
REGISTRY: dict[str, dict] = {
    ...
    "nanmax": {
        ...
        "module": "numpy",
        "complex_factor": 2.0,
        "notes": "Maximum ignoring NaNs. Billed exactly like its plain sibling at EVERY dtype: numpy takes a fast path for any plain non-object ndarray (which is every operand flopscope hands it), reducing with fmax/fmin and then testing the REDUCED OUTPUT with np.isnan(res).any(). It never reaches _replace_nan, so there is no input-sized NaN test pass to charge -- unlike the other factory-built nan* reductions, which do carry one on float/complex input.",
    },
    "nanmin": {
        "category": "counted_reduction",
        "module": "numpy",
        "complex_factor": 2.0,
        "notes": "Minimum ignoring NaNs. Billed exactly like its plain sibling at EVERY dtype: numpy takes a fast path for any plain non-object ndarray (which is every operand flopscope hands it), reducing with fmax/fmin and then testing the REDUCED OUTPUT with np.isnan(res).any(). It never reaches _replace_nan, so there is no input-sized NaN test pass to charge -- unlike the other factory-built nan* reductions, which do carry one on float/complex input.",
    },
    "nanmean": {
        "category": "counted_reduction",
        "module": "numpy",
