...
| `gradient` | base: `sum over the axes axis= selects (all axes when axis=None) of 2·S` (`S = f.size`; one output value per element along each axis — interior differences and both boundaries — at ~2 FLOPs each, so cost is independent of axis length); each coord-array axis adds a spacing surcharge (uniform: `+3(L−1)`; non-uniform: `+3S(L−2)/L + 10(L−2) + 3(L−1) + 4S/L`, `L = f.shape[axis]`) | DERIVED | `_pointwise.py:gradient` |
