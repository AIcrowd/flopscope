...
| **No free symmetry tag** | a `SymmetricTensor` cannot be minted over data that doesn't actually satisfy the claimed group: top-level construction — `as_symmetric(data, symmetry=…)` and the bare `SymmetricTensor(data, symmetry=…)` constructor alike — validates the claim and raises `SymmetryError` on a mismatch, charging `k·(7n − 1)` (`k` = non-identity generators, `n = data.size`) for a genuine one. Only symmetry a flopscope op derives *internally* (e.g. `exp` propagating the tag of an already-validated operand, or a slice/transpose view) is exempt from re-paying — it never reaches an unvalidated, caller-supplied claim | `tests/test_symmetric_tensor_new_validation.py`; `tests/test_symmetric_cost.py` (`k·(7n−1)` rate) |
