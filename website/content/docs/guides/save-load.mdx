...
*Store weights in a portable, pickle-free file. Loading them back is free; saving bills the bytes you write.*
...
Loading is **free**: `fnp.load` charges 0 FLOPs regardless of file size. Saving is
not — `fnp.save`, `fnp.savez`, and `fnp.savez_compressed` each charge 4 FLOPs per
element written, plus 4 FLOPs per byte of the array names themselves and of a
serialized `__meta__` block, so there is no free way to smuggle data through any of
those channels.
...
> **Note:** flopscope refuses `object` dtype everywhere, not just at save —
> building one with `fnp.array(..., dtype=object)` already raises
> `UnsupportedDtypeError`. See the object-dtype note in the [FLOP counting
> model](/docs/understanding/flop-counting-model). The file format itself
> accepts only numeric dtypes (float, int, bool, complex); `str_`, `bytes_`,
> `datetime64`, `timedelta64`, and structured dtypes are rejected by `save`
> with a `ValueError` even though they are billable everywhere else.
