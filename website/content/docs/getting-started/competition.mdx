...
For Phase 2 of the ARC White-Box Estimation Challenge, the
[official Rules](https://www.aicrowd.com/challenges/arc-white-box-estimation-challenge-2026/challenge_rules)
govern eligibility and take precedence over library documentation. Staying within
a metered FLOP budget does not by itself make a submission eligible. Do not pack
independent values into one machine element to obtain an accounting advantage,
even if every operation is metered. Choosing a lower-precision dtype or quantizing
values is not, by itself, this packing technique; all choices still have to satisfy
the Rules. See [On packing](/docs/understanding/flop-counting-model/#on-packing) for the
distinction between billing and eligibility.
...
accounting meaning. During evaluation, the remote global summary is a read-only
snapshot of authoritative server state.

The one-participant-process epoch is a WHEST evaluator deployment guarantee,
not a property of every standalone server. A conforming evaluator must launch
the server with `--token-fd`, retain the minted token only in its trusted control
channel, perform a post-smoke, pre-scoring reset, and replace the participant
whenever the server is replaced. Under that contract, smoke work is excluded
and participant code cannot reset the epoch because it never receives the
control token. Tokenless local/development servers intentionally accept
lifecycle control operations and are not hardened participant boundaries. An
incompatible peer that lacks authoritative summary support raises an actionable
error instead of returning partial or zero-filled data.

Both flat and namespaced summaries are constructed in `O(K)` time for the
operation and optional namespace buckets returned, independent of historical
call count. `budget_summary()` and `budget.summary()` format those same mappings.
Opting into `by_namespace=True` adds the namespace breakdown without changing
the flat budget, FLOP, operation, or timing totals.

Summary inspection contributes FLOPScope overhead. The current call takes its
snapshot first and commits its measured inspection overhead afterward, so that
overhead appears in the next snapshot or a later final-close snapshot. A final
close response cannot recursively include its own post-boundary serialization.

An explicit `BudgetContext` has three summary states:

- Before its first entry, `summary_dict()` returns the canonical empty mapping.
- While it is active, `summary_dict()` requests that authoritative server
  resource and preserves the client-owned context's existing end-to-end timing
  meaning by normalizing only the four top-level timing fields.
- After close, `summary_dict()` returns a defensive copy of the cached final
  mapping. Later session activity cannot change it.

Live scalar timing properties deliberately keep their existing
`None`/zero-until-close behavior. After close, all scalar timing properties are
read from that same cached mapping, so scalar, dictionary, and formatted views
cannot diverge. Client-only timing measurements are not sent to the server and
cannot change FLOPs, operations, namespaces, budgets, the global session
summary, or scoring.
...
in submitted code. Passing a generator, lambda, dict, or other non-array
object where an array is expected now fails the same way in-process and
remotely: NumPy coerces it to an `object`-dtype array and flopscope raises
`UnsupportedDtypeError` before anything is sent over the wire, because an
object cell can hold unbounded computation the cost model cannot price. Pass
materialized numeric arrays or built-in scalars instead.
