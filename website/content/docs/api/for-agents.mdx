...
- 31 operations are blocked entirely (config, state, text/legacy I/O); weight I/O (`save`/`load`/`savez`/`savez_compressed`) is **supported** and pickle-free
...
|----------|--------|----------|
| <StaticFileLink href="/llms.txt">llms.txt</StaticFileLink> | Markdown | Start here. Curated index of all doc pages with one-line descriptions. Under 4K tokens. |
| <StaticFileLink href="/llms-full.txt">llms-full.txt</StaticFileLink> | Markdown | Complete docs in one file. Use if your context window is large enough (~115KB). |
| <StaticFileLink href="/ops.json">ops.json</StaticFileLink> | JSON | Slim machine-readable index of all 508 operations. Query programmatically for names, filters, and detail URLs. |
...
| [API Reference](/docs/api/) | Markdown | Dense reference of every operation's cost and full operation inventory. |
...
- Get an operation's cost weight: read `weight` (the per-unit multiplier; the dtype
  rate is applied on top — see the [FLOP counting model](/docs/understanding/flop-counting-model))
...
Blocked: `geterr`, `seterr`, `savetxt`, `loadtxt`, and 27 others. These raise
`AttributeError`. (`save`/`load`/`savez`/`savez_compressed` are supported and
pickle-free — see the Saving & Loading guide.)

When in doubt, check `ops.json`, the relevant `/api-data/ops/<slug>.json`, or the [API Reference](/docs/api/).
...
Most linear algebra can be expressed as einsum. The cost follows the FMA=2
textbook convention — each FMA (fused multiply-add) counts as two operations.
`'ij,jk->ik'` with shapes `(m, k)` and `(k, n)` costs `2*m*k*n − m*n` FLOPs.
...
- Use `symmetry=` to declare a symmetric output:
  `fnp.einsum('ki,kj->ij', X, X, symmetry=(0, 1))`
- Wrap known-symmetric matrices with `flops.as_symmetric(data, symmetry=(0, 1))`
...
- [API Reference](/docs/api/) — full operation inventory and cost reference
- [Exploit Symmetry](/docs/guides/symmetry/) — detailed symmetry guide
