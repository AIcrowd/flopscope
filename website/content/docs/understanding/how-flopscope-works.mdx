...
Every successful deduction is recorded as an `OpRecord` in the budget's
operation log, capturing the operation name, input shapes, FLOP cost,
cumulative total, context start offset, backend duration, and Flopscope
overhead duration. The operation log remains available for per-call
diagnostics. Incremental
operation and namespace rollups, updated alongside the log, power summary
accessors without rescanning historical calls.
...
**Free operations** include allocation (`zeros`, `ones`, `empty`), shape manipulation (`reshape`, `transpose`, `squeeze`), indexing helpers (`indices`), and metadata queries (`shape`, `ndim`, `size`). These do not touch the budget.
...
When per-operation [weights](/docs/understanding/flop-counting-model/) are loaded, the analytical cost is multiplied by the operation's weight before deduction. This allows the cost model to reflect that `exp` is more expensive than `abs` in terms of actual hardware instructions, while keeping the base formulas simple and deterministic.
